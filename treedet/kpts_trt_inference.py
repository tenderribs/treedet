...
def main(args):
    ...
    image_files = [
        ...
        and os.path.isfile(os.path.join(args.images_path, f))
    ]
    ...
    for image_file in image_files:
        ...
        d_output = cuda.mem_alloc(
            int(1 * np.prod(engine.get_binding_shape(1)) * 4)
        ...
        output = np.empty(
            engine.get_binding_shape(1), dtype=np.float32
        )  # Assuming output is float32
    ...
    print(
        f"Total inference time: {total_inference}s for {len(image_files)} images, avg {total_inference / len(image_files)}s per image"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Score threshold to filter the results.",
    )
